...
INNER JOIN paciente pac ON eq.IdEspecialista=pac.IdMedico INNER JOIN bitacora bita ON pac.IdPaciente=bita.IdPaciente WHERE per.IdPersona='".$IdMedico."' AND bita.FechaHora BETWEEN '".$DataIni." 00:00:00' AND '".$DataFin." 23:59:59') CountBitacoras
FROM persona per 
INNER JOIN trabajador tra ON per.IdPersona=tra.IdPersona 
INNER JOIN rol ON tra.IdRol=rol.IdRol
INNER JOIN equipo eq ON tra.IdTrabajador=eq.IdEspecialista OR tra.IdTrabajador=eq.IdAsistente 
...
WHERE per.IdPersona='".$IdMedico."'  AND noti.FechaNotificacion BETWEEN '".$DataIni." 00:00:00' AND '".$DataFin." 23:59:59' AND noti.Mensaje='Vaya a servicio de urgencia.'" );
$resultado=array();
